...
function loadState() {
    ...
    if (fs.existsSync(STATE_FILE)) {
        try {
            ...
            console.log("Invalid state.json. Creating new state.");
        }
    }
...
async function sendTG(nation) {
    ...
    // Do not treat an obvious API error as success
    if (
        /<ERROR>|<error>|error=/i.test(response)
    ) {
        throw new Error(
            `NationStates API returned an error: ${response}`
        );
    }
